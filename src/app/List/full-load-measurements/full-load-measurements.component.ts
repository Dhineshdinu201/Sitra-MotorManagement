...
@Component({
  selector: 'app-full-load-measurements',
  templateUrl: './full-load-measurements.component.html',
  styleUrls: ['./full-load-measurements.component.css']
})
export class FullLoadMeasurementsComponent implements OnInit {
  ...
  Delete(data) {
    ...
    this.dataservice.DeleteFullLoadMeasurementsData(departmentMaster).subscribe(resp => {
      if (resp == true) {
  ...
  View(data) {
    this.apiservice.setEditMasterData(data);
    this.bsModalRef = this.modalService.show(FullLoadMeasuremntViewComponent);
  }
